 ...
 */
function DataCursor(chain, offset, transform) {
  this._chain = chain;
 ...
 */
DataCursor.prototype.read = function(max_iterations, callback) {
  ...
  var insight_size = max_iterations < 0 ? this._chain.size() : this._transform.minInsightSize * max_iterations;
  this._chain.insight(this._offset, insight_size, function(err, buf, buf_offset, avail) {
    ...
    for (var j = 0; max_iterations < 0 || j < max_iterations; ++j) {
      ...
      if (chunk_size <= 0) {
        // chunk size is 0? we cannot continue to reading data. this is good way for
        // transform to report logical end of data.
        callback(null, null);
        return;
      }
      ...
      if (decoded[0] != undefined) {
        ...
        // continue, skipping as many bytes as chuck contains (as detected by sizeFunc)
